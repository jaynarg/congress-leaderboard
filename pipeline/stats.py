...
from .stages import STAGE_LABELS, bill_stage, stage_events

SITE_DIR = config.DATA_DIR / "site"
AI_DIR = config.DATA_DIR / "ai-summaries"
TAG_RE = re.compile(r"<[^>]+>")
SUMMARY_CHARS = 400
RECENT_WINDOW_DAYS = 90
# Congressional Review Act disapprovals are a genre of their own among enacted laws.
CRA_MARKERS = ("congressional disapproval under chapter 8 of title 5", "disapproving the rule submitted by")
...
STAGE_ORDER_INDEX = {k: i for i, k in enumerate(STAGE_LABELS)}
...
def summary_for(bill):
    """CRS summary if there is one, otherwise the AI summary written by pipeline.summarize."""
    crs = crs_summary(bill)
    if crs:
        return crs, "crs"
    try:
        ai = json.loads((AI_DIR / bill["type"] / f"{bill['number']}.json").read_text(encoding="utf-8"))
    except (FileNotFoundError, json.JSONDecodeError):
        return None, None
    text = ai.get("text")
    return (text, "ai") if text else (None, None)


def _is_cra(title):
    t = (title or "").lower()
    return any(marker in t for marker in CRA_MARKERS)
...
def build(data_dir=None):
    ...
    skipped = 0
    laws, recent = [], []
    cutoff = (datetime.now(timezone.utc).date() - timedelta(days=RECENT_WINDOW_DAYS)).isoformat()
    for path in bill_files:
        ...
        summary_text, summary_source = summary_for(bill)
        events = stage_events(bill)
        card = {
            "id": bill["id"], "number": f"{bill['type'].upper()} {bill['number']}",
            "type": bill["type"], "ranked": ranked, "title": bill.get("title"),
            "introducedDate": intro, "policyArea": bill.get("policyArea"),
            "sponsorId": sponsor_id, "sponsorName": by_id[sponsor_id]["name"],
            "sponsorCaucus": sponsor_caucus, "sponsorState": by_id[sponsor_id]["state"],
            "chamber": by_id[sponsor_id]["chamber"],
            "cosponsors": len(cosponsors), "crossPartyCosponsors": cross,
            "bipartisan": bipartisan, "summary": summary_text, "summarySource": summary_source,
            "url": bill["congressGovUrl"],
        }
        if stage == "became_law":
            enacted = events.get("became_law")
            law_number = (bill.get("laws") or [{}])[0].get("number")
            laws.append(dict(card, lawNumber=law_number, enactedDate=enacted,
                             daysToEnact=_days_between(intro, enacted),
                             cra=_is_cra(bill.get("title"))))
        for event_stage, event_date in events.items():
            if event_stage != "introduced" and event_date >= cutoff:
                recent.append(dict(card, stage=event_stage, date=event_date))
        ...
        a["sponsoredBills"].append({
            ...
            "summary": summary_text, "summarySource": summary_source,
            "url": bill["congressGovUrl"],
        })

    laws.sort(key=lambda law: (law["enactedDate"] or "", law["id"]), reverse=True)
    recent.sort(key=lambda e: (e["date"], STAGE_ORDER_INDEX.get(e["stage"], 0)), reverse=True)
    _write_json(SITE_DIR / "laws.json", laws)
    _write_json(SITE_DIR / "recent.json", recent)
    ...
    meta = {
        ...
        "policyAreas": sorted({law["policyArea"] for law in laws if law["policyArea"]}),
        "stageLabels": STAGE_LABELS,
        "definitions": {
            "summaries": "Congressional Research Service summaries are shown where they exist. Where they do not, a summary generated by Claude (Anthropic) from the bill text is shown and labelled as such.",
